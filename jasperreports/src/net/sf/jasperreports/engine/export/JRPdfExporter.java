 ...
 * @version $Id: JRPdfExporter.java,v 1.2 2017/09/26 16:26:13 eduardo Exp $
 */
public class JRPdfExporter extends JRAbstractExporter
{
	...
	protected JRPdfExporterContext exporterContext = new ExporterContext();
        ...
        protected int mlTextFieldName=1;
	/**
	 *
	 */
	...
	{
                Logger.getLogger(JRPdfExporter.class.getName()).log(Level.SEVERE, null, "Exportar PDF creativa3d");
                registerFonts();
	...
	public void exportText(JRPrintText text) throws DocumentException
	{
		...
		if (text.getModeValue() == ModeEnum.OPAQUE)
		{
			...
			pdfContentByte.fill();
                ...
                if(text.isPdfEmbedded()){
                    try {
                        
                        TextField title = new TextField(pdfWriter
                                , new Rectangle(
                                    x + leftPadding
                                    , jasperPrint.getPageHeight()
                                        - y
                                    ...
                                    , jasperPrint.getPageHeight()
                                        ...
                                        - height
                                        + bottomPadding)
                                , "name"+String.valueOf(mlTextFieldName));
                        title.setText(text.getFullText());
                        title.setAlignment(horizontalAlignment);
//                                title.setFont(new LocalFontMapper().awtToPdf(text.) );
                        title.setFontSize(text.getFontSize());
                        title.setOptions(TextField.MULTILINE);
                        
                        pdfWriter.addAnnotation(title.getTextField());
                        mlTextFieldName++;
